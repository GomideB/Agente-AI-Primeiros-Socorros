...
3.  Troque as Chaves de acesso: No código troque as chaves de API dos "Secrets" que você configurou no Passo 2.
   - As linhas - ```python os.environ["GOOGLE_API_KEY"] = userdata.get('GOOGLE_API_KEY') e os.environ["Maps_API"] = userdata.get('Maps_API')```python - **na célula 3 do código** são responsáveis por isso.
